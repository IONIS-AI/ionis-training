#!/usr/bin/env python3
"""
train_v21.py — IONIS V21 Training

V21 adds:
    - vertex_lat: Path polar exposure for storm sensitivity (dnn_dim 11→12)
    - RBN Full integration: Pressure Vessel experiment (rbn_full_sample=20M)

All configuration loaded from config_v21.json.
...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "config_v21.json")
...
def train():
    ...
    # V21: Optional RBN Full (Pressure Vessel experiment)
    ...
    # ── Features (from train_common — now includes vertex_lat) ──
    log.info("")
    log.info("Engineering features...")
    log.info(f"  dnn_dim={CONFIG['model']['dnn_dim']} (includes vertex_lat)")
    ...
    log.info("")
    log.info("SUCCESS CRITERIA (V16 Physics Laws):")
    ...
    if p_pass and kp_pass and sfi_pass:
        log.info("")
        log.info(">>> V21 TRAINING: SUCCESS <<<")
        if rbn_full_sample > 0:
            log.info(">>> PRESSURE VESSEL THEORY: CONFIRMED <<<")
            log.info(">>> RBN Full integrated with V16 Physics Laws intact <<<")
    else:
        log.info("")
        log.info(">>> V21 TRAINING: FAILED <<<")
        log.info(">>> Review training logs <<<")
