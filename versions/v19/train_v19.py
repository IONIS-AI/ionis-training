#!/usr/bin/env python3
"""
train_v19.py — IONIS V19 Rosetta Stone (Per-Source Normalization)

V19 reverts to V16's per-source Z-normalization, which was correct by design.
V18's global normalization failed because it treated incompatible measurement
scales (WSPR, RBN, Contest) as the same "dB" unit.

Key insight from Gemini:
    - WSPR: True SNR in 2500Hz bandwidth (calibrated)
    - RBN: "Skimmer SNR" (relative to site-variable noise floor)
    - Contest: Synthetic SNR (binary worked/not → +10/0 dB)

Per-source normalization means the model learns GEOMETRY and PHYSICS
without getting confused by INSTRUMENT CALIBRATION.

The "Rosetta Stone" decoder at inference time:
    - Model outputs Z-score (σ) = signal quality relative to average
    - Denormalize using source-appropriate constants based on mode

Data sources:
    - WSPR signatures (20M sample)
    - RBN signatures (20M sample)
    - RBN DXpedition signatures (91K × 50)
    - Contest signatures (6.3M)

Storm upsample retained: 10x for Kp >= 5.

Architecture: IonisV12Gate (unchanged — 203,573 params)
Checkpoint: versions/v19/ionis_v19.pth
"""
...
MODEL_FILE = "ionis_v19.pth"
MODEL_PATH = os.path.join(SCRIPT_DIR, MODEL_FILE)
PHASE = "V19 Rosetta Stone (Per-Source Norm)"
VERSION = "v19-per-source"
...
log = logging.getLogger('ionis-v19')
...
def load_combined_data():
    ...
    log.info("")
    log.info("=== V19 PER-SOURCE NORMALIZATION (Rosetta Stone) ===")
...
def main():
    log.info(f"IONIS V19 Rosetta Stone | {PHASE}")
    ...
    log.info(f"Normalization: PER-SOURCE Z-score (V19 Rosetta Stone)")
    ...
    log.info("")
    log.info("=== V19 ROSETTA STONE SUMMARY ===")
