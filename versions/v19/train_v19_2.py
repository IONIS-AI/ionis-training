#!/usr/bin/env python3
"""
train_v19_2.py — IONIS V19.2 "Differential Learning Rates"

V19.1 Two-Stage Freeze failed — frozen random weights = noisy baseline,
sidecars couldn't learn against that noise.

V19.2 remediation per Gemini:
    - Train EVERYTHING from epoch 1 (no freezing)
    - Differential Learning Rates:
        - Trunk + Gates: 1e-4 (slow, complex geography)
        - Sidecars: 1e-2 (100x faster, capture physics first)

By giving sidecars 100x larger step size, they explain variance using
SFI/Kp FIRST before the trunk can cannibalize those gradients.

Expected at Epoch 1:
    - Pearson: ~+0.30 (trunk learning)
    - Kp9-: > +0.5σ (sidecar learning fast)

Architecture: IonisV12Gate (unchanged — 173,173 params)
Checkpoint: versions/v19/ionis_v19_2.pth
"""
...
MODEL_FILE = "ionis_v19_2.pth"
MODEL_PATH = os.path.join(SCRIPT_DIR, MODEL_FILE)
PHASE = "V19.2 Differential LR (Physics First)"
VERSION = "v19.2-differential-lr"
...
STORM_UPSAMPLE_FACTOR = 20  # Keep 20x from V19.1

# Differential Learning Rates (V19.2 key innovation)
# Using V16's battle-tested rates (9975WX recommendation)
TRUNK_LR = 1e-5      # Slow: complex geography (V16 proven)
SIDECAR_LR = 1e-3    # 100x faster: capture physics first (V16 proven)
WEIGHT_DECAY = 1e-5  # V16 proven (Gemini's 1e-2 may cause instability)
...
log = logging.getLogger('ionis-v19.2')
...
def load_combined_data():
    ...
    log.info("")
    log.info("=== V19.2 PER-SOURCE NORMALIZATION (Rosetta Stone) ===")
...
def main():
    log.info(f"IONIS V19.2 Differential LR | {PHASE}")
    ...
    log.info(f"Learning rates: Trunk={TRUNK_LR}, Sidecars={SIDECAR_LR} (100x faster)")
    log.info("")
    ...
    log.info(f"Trainable: {trainable_params:,} / {total_params:,}")
    log.info("")

    # ── V19.2 Differential Learning Rate Configuration ──
    # Trunk + Gates learn SLOWLY (Geometry is complex but stable)
    # Sidecars learn FAST (Physics signals are simple but weak)
    ...
    trunk_param_count = sum(p.numel() for p in trunk_params)
    gate_param_count = sum(p.numel() for p in gate_params)
    sidecar_param_count = sum(p.numel() for p in sidecar_params)

    log.info("=== V19.2 DIFFERENTIAL LEARNING RATES ===")
    log.info(f"  Trunk ({trunk_param_count:,} params):   lr={TRUNK_LR}")
    log.info(f"  Gates ({gate_param_count:,} params):    lr={TRUNK_LR}")
    log.info(f"  Sidecars ({sidecar_param_count:,} params): lr={SIDECAR_LR} (100x boost)")
    log.info("")
    ...
    # Physics assessment
    log.info("")
    log.info("=== V19.2 PHYSICS ASSESSMENT ===")
    ...
    log.info("")
    log.info("=== V19.2 ROSETTA STONE SUMMARY ===")
