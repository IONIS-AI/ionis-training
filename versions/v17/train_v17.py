#!/usr/bin/env python3
"""
train_v17.py — IONIS V17 RBN Grid Enrichment

V17 adds real machine-measured SNR from RBN skimmers to complete the curriculum:
    ...
    - Rare Layer:    RBN DXpedition signatures (91K × 50 upsampled)

RBN grid enrichment:
    - 56.7M signatures from 2.18B CW/RTTY/PSK31 spots
    - Grid resolution via wspr.callsign_grid (Rosetta Stone)
    - Real SNR from skimmer hardware (not anchored like contest)
    - Fills the middle of the dynamic range between WSPR floor and contest ceiling

Curriculum sequence:
    - V14-TP: Balloon filter A/B test (baseline)
    - V15 Diamond: Clean WSPR + DXpedition rare paths
    - V16 Contest: + Contest anchoring (ceiling proof)
    - V17: + RBN full signatures (middle layer, real SNR)

Architecture: IonisV12Gate (unchanged — 203,573 params)
Target: Z-normalized median_snr (per-source per-band)
Weight: spot_count (normalized within source)

Checkpoint: models/ionis_v17.pth
"""
...
MODEL_FILE = "ionis_v17.pth"
PHASE = "V17 RBN Grid Enrichment: Floor + Middle + Ceiling"
VERSION = "v17-rbn-enrichment"
...
RBN_FULL_SAMPLE = 20_000_000  # 20M RBN signatures (middle) — matches WSPR
RBN_DX_SAMPLE = None          # All 91K DXpedition signatures (rare paths)
CONTEST_SAMPLE = None         # All 6.3M contest signatures (ceiling)

# Upsampling: repeat DXpedition data to balance contribution
# 20M / 91K ≈ 220x, but we use 50x to avoid dominating
RBN_DX_UPSAMPLE_FACTOR = 50   # 91K × 50 = 4.55M effective DXpedition rows
...
# Per-source per-band normalization constants
# wspr: from WSPR signatures (floor, ~-18 dB)
# rbn_dx: from RBN DXpedition signatures (rare paths, ~17 dB)
# rbn_full: from RBN full signatures (middle, ~17 dB, real machine SNR)
NORM_CONSTANTS = {
    102: {'wspr': (-18.04, 6.9), 'rbn_dx': (11.86, 6.23), 'rbn_full': (18.09, 9.30)},
    103: {'wspr': (-17.90, 6.9), 'rbn_dx': (16.94, 6.71), 'rbn_full': (19.59, 8.64)},
    104: {'wspr': (-17.60, 7.1), 'rbn_dx': (16.91, 7.1),  'rbn_full': (15.91, 6.99)},
    105: {'wspr': (-17.34, 6.6), 'rbn_dx': (18.36, 6.6),  'rbn_full': (18.76, 8.10)},
    106: {'wspr': (-18.07, 6.5), 'rbn_dx': (16.96, 6.5),  'rbn_full': (16.44, 7.44)},
    107: {'wspr': (-17.53, 6.7), 'rbn_dx': (18.47, 6.7),  'rbn_full': (17.63, 8.06)},
    108: {'wspr': (-18.35, 7.0), 'rbn_dx': (16.66, 7.0),  'rbn_full': (16.04, 7.77)},
    109: {'wspr': (-18.32, 6.6), 'rbn_dx': (16.37, 6.6),  'rbn_full': (16.98, 8.16)},
    110: {'wspr': (-18.76, 6.6), 'rbn_dx': (15.07, 6.6),  'rbn_full': (16.35, 8.15)},
    111: {'wspr': (-17.86, 6.5), 'rbn_dx': (15.79, 6.5),  'rbn_full': (16.26, 8.07)},
}
...
log = logging.getLogger('ionis-v17')
...
# ── Model (unchanged from V12/V13/V15/V16) ───────────────────────────────────
...
def load_combined_data():
    ...
    # ── RBN Full Signatures (middle layer — NEW in V17) ──
    ...
    rbn_full_snr = normalize_snr(rbn_full_df, 'rbn_full')
    rbn_dx_snr = normalize_snr(rbn_dx_df, 'rbn_dx')
    # Contest uses WSPR normalization constants (anchored dB → Z-score)
    contest_snr = normalize_snr(contest_df, 'wspr')

    log.info(f"  WSPR normalized:     mean={wspr_snr.mean():.3f}, std={wspr_snr.std():.3f}")
    log.info(f"  RBN Full normalized: mean={rbn_full_snr.mean():.3f}, std={rbn_full_snr.std():.3f}")
    log.info(f"  RBN DX normalized:   mean={rbn_dx_snr.mean():.3f}, std={rbn_dx_snr.std():.3f}")
    ...
    rbn_dx_snr_up = np.tile(rbn_dx_snr, RBN_DX_UPSAMPLE_FACTOR)
    ...
    y_combined = np.concatenate([wspr_snr, rbn_full_snr, rbn_dx_snr_up, contest_snr_up]).reshape(-1, 1)
    w_combined = np.concatenate([wspr_w, rbn_full_w, rbn_dx_w_up, contest_w_up]).reshape(-1, 1)

    total = len(X_combined)
    log.info(f"Combined dataset: {total:,} rows")
    log.info(f"  WSPR (floor):       {len(wspr_X):,} ({100*len(wspr_X)/total:.1f}%)")
    log.info(f"  RBN Full (middle):  {len(rbn_full_X):,} ({100*len(rbn_full_X)/total:.1f}%)")
    log.info(f"  RBN DX (rare):      {len(rbn_dx_snr_up):,} ({100*len(rbn_dx_snr_up)/total:.1f}%)")
    log.info(f"  Contest (ceiling):  {len(contest_snr_up):,} ({100*len(contest_snr_up)/total:.1f}%)")
...
def main():
    log.info(f"IONIS V17 RBN Enrichment | {PHASE}")
    ...
    log.info(f"New in V17: RBN grid-enriched signatures (56.7M → 20M sample)")
    log.info("")
    ...
    # Convert to approximate dB (avg std ≈ 7.5 dB for RBN-heavy mix)
    avg_std_db = 7.5
    ...
    log.info("")
    log.info("=== V17 RBN Enrichment Summary ===")
    log.info(f"V16 Contest (baseline):   Pearson +0.4873, PSK Reporter 84.14%")
    log.info(f"V17 RBN Enrichment:       Pearson {best_pearson:+.4f}")
    log.info("")
    log.info("Curriculum layers:")
    log.info("  Floor:   WSPR -28 dB (measured low-power)")
    log.info("  Middle:  RBN ~18 dB (real machine-measured CW/RTTY)")
    log.info("  Ceiling: Contest +10/0 dB (anchored human QSO)")
    log.info("  Rare:    DXpedition (152 DXCC, 50x upsampled)")
    log.info("")
