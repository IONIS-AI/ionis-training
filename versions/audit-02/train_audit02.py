#!/usr/bin/env python3
"""
train_audit02.py — AUDIT-02: Xavier Uniform Init (No Defibrillator)

HYPOTHESIS: If +0.48σ still appears with Xavier init, it is clamp-dependent.
            (AUDIT-01 already confirmed this — AUDIT-02 is the control.)

RECIPE: V22-gamma (dnn_dim=15, no IRI features)
CHANGE: Replace Defibrillator init with Xavier Uniform
CLAMP: Original [0.5, 2.0]
DURATION: 30 epochs

EXPECTED: SFI will still hit +0.48σ because clamp floor is unchanged.
"""
...
from common.train_common import (
    ...
    SignatureDataset,
    engineer_features,
    clamp_sidecars,  # Original [0.5, 2.0] clamp
    get_optimizer_groups,
    load_source_data,
    log_config,
)

# ── AUDIT-02: XAVIER INIT (NO DEFIBRILLATOR) ────────────────────────────────


def init_xavier(model):
    """
    Initialize sidecar weights with Xavier Uniform (standard practice).

    AUDIT-02: Testing if the +0.48 fixed point was init-dependent.
    (AUDIT-01 already proved it was clamp-dependent, this is the control.)
    """
    with torch.no_grad():
        for sidecar in [model.sun_sidecar, model.storm_sidecar]:
            nn.init.xavier_uniform_(sidecar.fc1.weight)
            nn.init.xavier_uniform_(sidecar.fc2.weight)
            nn.init.zeros_(sidecar.fc1.bias)
            nn.init.zeros_(sidecar.fc2.bias)

    log = logging.getLogger("ionis-audit-02")
    log.info("Xavier Uniform init applied (NO Defibrillator)")
    log.info("  fc1/fc2 weights: Xavier Uniform")
    log.info("  fc1/fc2 biases: zeros")
...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "config_audit02.json")
    ...
    try:
        ...
        query = f"""
            INSERT INTO training.runs (
                ...
                {features_array},
                {val_rmse}, {val_pearson}, {sfi_benefit}, {storm_cost}, {best_epoch},
                'AUDIT-02: Xavier init - COMPLETED'
            )
        """
        requests.post(AUDIT_HOST, params={"query": query}, timeout=5)
    except Exception as e:
...
def train():
    ...
    log.info("=" * 70)
    log.info("AUDIT-02: XAVIER UNIFORM INIT (NO DEFIBRILLATOR)")
    log.info("=" * 70)
    log.info("")
    log.info("HYPOTHESIS: If +0.48 still appears, it is clamp-dependent.")
    log.info("EXPECTED: SFI will hit +0.48 (clamp floor unchanged at [0.5, 2.0]).")
    log.info("(AUDIT-01 already confirmed clamp is the cause — this is control.)")
    log.info("")
    ...
    # ── AUDIT-02: XAVIER INIT (NOT DEFIBRILLATOR) ──
    init_xavier(model)
    ...
    # Log initial weights
    log.info("")
    log.info("=== INITIAL WEIGHTS (POST-XAVIER) ===")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if is_best:
            ...
            metadata = {
                ...
                'audit': 'AUDIT-02: Xavier init (no Defibrillator)',
    ...
    log.info("")
    if abs(best_sfi - 0.48) < 0.05:
        log.info(">>> EXPECTED RESULT <<<")
        log.info(f">>> SFI hit {best_sfi:+.4f}σ (~+0.48) with Xavier init <<<")
        log.info(">>> Confirms AUDIT-01: +0.48 is clamp floor, not init-dependent <<<")
    else:
        log.info(">>> UNEXPECTED RESULT <<<")
        log.info(f">>> SFI at {best_sfi:+.4f}σ (expected ~+0.48) <<<")
        log.info(">>> Further investigation needed <<<")
