#!/usr/bin/env python3
"""
train_v23.py — IONIS V23 Training (PyIRI Integration)

V23 adds:
    - Pre-computed IRI-2020 atlas (foF2, hmF2, foE) for localized ionospheric physics
    - foF2_freq_ratio: THE feature — ratio > 1 means operating freq < MUF (propagation possible)
    - foE_mid: D-layer indicator for low-band absorption
    - hmF2_mid: F-layer height for skip distance estimation

Key insight: V22-gamma learned band×time physics but low bands (160m/80m) still failed
to show correct magnitude. The model needs localized ionospheric parameters, not just
global SFI. When foF2_freq_ratio > 1, the band is physically open at that path/hour.

All configuration loaded from config_v23.json.
...
SFI Sidecar Watch:
    - If SFI sigma drops toward zero: foF2 has fully replaced it
    - If SFI sigma stays positive: SFI carries info beyond foF2
...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "config_v23.json")
...
def train():
    ...
    # ── Load IRI Atlas (V23 specific) ──
    log.info("")
    log.info("=== IRI ATLAS (V23) ===")
    ...
    if not os.path.exists(iri_path):
        ...
        log.error("Run: scp 10.60.1.1:/mnt/ai-stack/ionis-ai/iri_lookup.npz data/")
        return

    iri_data, iri_grid_index, iri_sfi_buckets = load_iri_atlas(iri_path)
    log.info(f"  Grids: {len(iri_grid_index):,}")
    log.info(f"  SFI buckets: {iri_sfi_buckets}")
    ...
    # ── Load Data (with day_of_year for V22+ features) ──
    ...
    # Optional RBN Full (not recommended for V23 — one variable at a time)
    rbn_full_sample = CONFIG["data"].get("rbn_full_sample", 0)
    if rbn_full_sample > 0:
        log.info(f"Loading RBN Full: {rbn_full_sample:,} rows")
    ...
    # ── Features (V23: V22 features + IRI atlas) ──
    log.info("")
    log.info("Engineering features...")
    log.info(f"  dnn_dim={CONFIG['model']['dnn_dim']} (V22 + foF2_freq_ratio, foE_mid, hmF2_mid)")
    ...
    log.info("WATCH: SFI sidecar — if it drops toward zero, foF2 is replacing it")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for bx, by, bw in train_loader:
            ...
            # NaN guard (per red team recommendations)
            assert not torch.isnan(bx).any(), "NaN in input tensor!"
    ...
    log.info("")
    log.info("SFI SIDECAR WATCH:")
    if best_sfi < 0.3:
        log.info(f"  SFI sigma dropped to {best_sfi:+.3f}σ — foF2 may be replacing it")
        log.info("  Consider removing SFI sidecar in V24 if foF2_freq_ratio carries the signal")
    else:
        log.info(f"  SFI sigma stable at {best_sfi:+.3f}σ — SFI carries info beyond foF2")

    if p_pass and sfi_pass:
        log.info("")
        log.info(">>> V23 TRAINING: SUCCESS <<<")
        log.info(">>> Run TST-900 to verify band×time discrimination <<<")
    else:
        log.info("")
        log.info(">>> V23 TRAINING: NEEDS REVIEW <<<")
        log.info(">>> Check training logs <<<")

    log.info("")
    log.info("V22-gamma REFERENCE: Pearson +0.459, Kp +2.81σ, SFI +0.48σ, TST-900 9/11")
    log.info("V23 TARGET: TST-900 >= 10/11 (foF2_freq_ratio enables low-band physics)")
