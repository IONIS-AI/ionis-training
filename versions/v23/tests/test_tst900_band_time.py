#!/usr/bin/env python3
"""
test_tst900_band_time.py — IONIS V23 Band×Time Discrimination Tests

TST-900 Group: Verify band×time-of-day behavior matches ionospheric reality.

V23 builds on V22-gamma with PyIRI atlas integration:
  - dnn_dim=18 (V22's 15 + 3 IRI features)
  - IRI features: foF2_freq_ratio, foE_mid, hmF2_mid
  - Same physics gates, just enhanced with IRI ionospheric parameters
...
V23_DIR = os.path.dirname(SCRIPT_DIR)
VERSIONS_DIR = os.path.dirname(V23_DIR)
...
from model import (
    ...
    latlon_to_grid4, sfi_bucket, sfi_bucket_to_index,
)

# ── Load Config ──────────────────────────────────────────────────────────────

V23_CONFIG = os.path.join(V23_DIR, "config_v23.json")
V23_CKPT = os.path.join(V23_DIR, "ionis_v23.safetensors")
...
def load_iri_atlas():
    """Load the pre-computed IRI-2020 atlas (CoW-safe .npz)."""
    iri_path = os.path.join(TRAINING_DIR, CONFIG["iri_atlas"]["path"])
    print(f"  Loading IRI atlas from {iri_path}...")
...
def get_iri_params(lat, lon, hour_utc, month, sfi):
    """Look up IRI parameters for a single point."""
    ...
    sfi_idx = sfi_bucket_to_index(sfi_bucket(sfi))
...
def build_features_v23(tx_lat, tx_lon, rx_lat, rx_lon, freq_hz, sfi, kp,
                       hour_utc, month, day_of_year):
    """Build a V23 feature vector (18 DNN + SFI + Kp = 20 total)."""
    ...
    # V23 IRI features (indices 15-17)
    # Get IRI params at midpoint
...
def main():
    ...
    print("=" * 60)
    print("  IONIS V23 — TST-900 Band x Time Discrimination Tests")
    print("=" * 60)
    print(f"\n  Model Version: {MODEL_VERSION}")
    print(f"  DNN Dim: {DNN_DIM} (V22-gamma base + 3 IRI features)")
    print("  These tests verify band x time discrimination.")
    ...
    if passed == total:
        ...
        print("  V23 shows correct band x time discrimination.")
        return 0
    else:
        print(f"\n  {total - passed} TEST(S) FAILED")
        print("  Review failed tests for physics issues.")
        return 1
