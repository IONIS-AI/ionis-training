#!/usr/bin/env python3
"""
train_v25.py — IONIS V25-alpha Training

V25-alpha is V22-gamma with ONE change:
    - Sun sidecar input: SFI (1D) → SFI + freq_log (2D)

This gives the sun sidecar frequency awareness so it can learn:
    - SFI × high frequency → large positive output (10m benefits from SFI)
    - SFI × low frequency → small positive output (160m benefits less)
...
Architecture constraints (non-negotiable — V16 Physics Laws):
    1. Architecture: IonisGateV25 (context-aware gates from trunk output)
...
Success criteria: TST-900 >= 10/11, Pearson >= +0.46, SFI shows band-dependent behavior
"""
...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "config_v25.json")
...
# ── V25 Model: MonotonicMLP with configurable input dimension ─────────────────

class MonotonicMLP(nn.Module):
    """
    Monotonically increasing MLP for physics constraints.

    V25 change: Now accepts `input_dim` parameter.
    - Storm sidecar: input_dim=1 (Kp only)
    - Sun sidecar: input_dim=2 (SFI + freq_log)
    """
    ...
    def forward(self, x):
        # Monotonicity enforced via absolute value of weights
...
class IonisGateV25(nn.Module):
    """
    IONIS V25 Model — gated sidecars with frequency-aware sun sidecar.

    The ONLY change from V22-gamma's IonisGate:
        - Sun sidecar input: 1D (SFI) → 2D (SFI + freq_log)
        - MonotonicMLP for sun: input_dim=2

    Everything else identical:
        - Trunk: 15-dim → 512 → 256
        - Base head: 256 → 128 → 1
        - Scaler heads: 256 → 64 → 1
        - Storm sidecar: 1D (Kp penalty)
        - Gates from trunk output (256-dim)
    """

    def __init__(self, dnn_dim=15, sidecar_hidden=8, sfi_idx=15, kp_penalty_idx=16,
                 gate_init_bias=None, sun_sidecar_input_dim=2):
        super().__init__()
        ...
        self.sun_sidecar_input_dim = sun_sidecar_input_dim
        ...
        # V25 change: Sun sidecar takes 2D input (SFI + freq_log)
        self.sun_sidecar = MonotonicMLP(hidden_dim=sidecar_hidden, input_dim=sun_sidecar_input_dim)
        # Storm sidecar unchanged: 1D input (Kp penalty)
    ...
    def forward(self, x):
        ...
        # V25: Extract freq_log (index 1) for sun sidecar
        x_freq_log = x[:, 1:2]  # freq_log is always at index 1
        x_sun_input = torch.cat([x_sfi, x_freq_log], dim=1)  # [batch, 2]
    ...
    def forward_with_gates(self, x):
        ...
        # V25: Extract freq_log for sun sidecar
        x_freq_log = x[:, 1:2]
        x_sun_input = torch.cat([x_sfi, x_freq_log], dim=1)
    ...
    def get_sun_effect(self, sfi_normalized, freq_log_normalized, device):
        """Get raw sun sidecar output for given SFI and freq_log values."""
        with torch.no_grad():
            x = torch.tensor([[sfi_normalized, freq_log_normalized]],
    ...
    def get_sun_effect_by_band(self, sfi_normalized, device):
        ...
        Returns dict mapping band name to SFI effect in sigma.
        This is the key diagnostic for V25 — should show band-dependent values.
        """
        band_freq_hz = {
            "160m": 1_836_600,
            "80m":  3_568_600,
        ...
        for band, freq_hz in band_freq_hz.items():
            ...
            results[band] = self.get_sun_effect(sfi_normalized, freq_log_norm, device)
...
def train():
    ...
    log.info("")
    log.info("=== V25-ALPHA: ONE VARIABLE CHANGE ===")
    log.info("  Sun sidecar input: SFI (1D) → SFI + freq_log (2D)")
    ...
    # V25 row count sanity check (must match V22-gamma: 38,760,089)
    expected_rows = 38_760_089
    if abs(n - expected_rows) > 100_000:
        log.warning(f"ROW COUNT MISMATCH: Expected ~{expected_rows:,}, got {n:,}")
        log.warning("This may indicate a data recipe deviation from V22-gamma!")
    ...
    # ── Model (V25: 2D sun sidecar input) ──
    model = IonisGateV25(
    ...
    log.info(f"Model: IonisGateV25 ({total_params:,} params)")
    log.info(f"  Sun sidecar: {CONFIG['model']['sun_sidecar_input_dim']}D input (SFI + freq_log)")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # V25: Get band-specific SFI effects
        ...
        if epoch in [1, 10, 25, 50, 75, 100]:
            log.info(f"  SFI by band (SFI=200 vs SFI=70): {sfi_effects}")
    ...
    if abs(best_sfi_10m - best_sfi_160m) > 0.1:
        log.info("  >>> SFI sidecar is band-aware! <<<")
    else:
    ...
    log.info(f"  Kp storm cost: {best_kp:+.3f}σ")

    log.info("")
    log.info("COMPARISON TO V22-GAMMA:")
    log.info("  V22-gamma: SFI = +0.48σ uniform across all bands")
    log.info(f"  V25-alpha: SFI = {best_sfi_10m:+.2f}σ (10m), {best_sfi_160m:+.2f}σ (160m)")

    log.info("")
    log.info(">>> Run TST-900 to verify band×time discrimination <<<")
    log.info(">>> Target: TST-900 >= 10/11 (improvement over V22-gamma's 9/11) <<<")
