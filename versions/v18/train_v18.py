#!/usr/bin/env python3
"""
train_v18.py — IONIS V18 Global Normalization Fix

V18 fixes the normalization issue that blocked V17:
    - V17 used per-source per-band Z-normalization, erasing physical dB differences
    - V18 uses single global Z-normalization in raw dB, preserving physical ordering

Two changes from V17:
    1. Global raw-dB normalization (not per-source)
    2. Storm upsample (10x for Kp >= 5 signatures)

Data sources (unchanged from V17):
    - Floor Layer:   WSPR signatures (-28 dB measured, 20M sample)
    - Middle Layer:  RBN signatures (~18 dB real machine-measured, 20M sample)
    - Ceiling Layer: Contest signatures (+10/0 dB anchored, 6.3M)
    - Rare Layer:    RBN DXpedition signatures (91K x 50 upsampled)

Expected outcomes:
    - global_mean: -5 to +5 dB (not ~0.4)
    - global_std: 15-25 dB (not ~1.5)
    - Predictions denormalize to real dB range (-30 to +30)
    - PSKR recall: 80-95% (not 99%)
    - Kp storm cost: > +3.0σ (restored from V17's +2.5σ)

Architecture: IonisV12Gate (unchanged — 203,573 params)
Target: Z-normalized median_snr (GLOBAL, not per-source)
Weight: spot_count (normalized within source)

Checkpoint: versions/v18/ionis_v18.pth
"""
...
MODEL_FILE = "ionis_v18.pth"
MODEL_PATH = os.path.join(SCRIPT_DIR, MODEL_FILE)
PHASE = "V18 Global Normalization Fix"
VERSION = "v18-global-norm"

# ClickHouse connection (DAC link to 9975WX)
CH_HOST = os.environ.get("CH_HOST", "10.60.1.1")
CH_PORT = int(os.environ.get("CH_PORT", "8123"))

# Sample sizes (unchanged from V17)
WSPR_SAMPLE = 20_000_000      # 20M WSPR signatures (floor)
RBN_FULL_SAMPLE = 20_000_000  # 20M RBN signatures (middle)
RBN_DX_SAMPLE = None          # All 91K DXpedition signatures (rare paths)
CONTEST_SAMPLE = None         # All 6.3M contest signatures (ceiling)

# Upsampling
RBN_DX_UPSAMPLE_FACTOR = 50   # 91K x 50 = 4.55M effective DXpedition rows
CONTEST_UPSAMPLE_FACTOR = 1   # Contest already has 6.3M rows

# NEW: Storm upsample to restore Kp sensitivity
STORM_UPSAMPLE_FACTOR = 10    # 10x upsample for Kp >= 5 signatures
...
# NOTE: NORM_CONSTANTS removed — V18 uses global normalization
...
log = logging.getLogger('ionis-v18')
...
# NOTE: normalize_snr() removed — V18 uses global normalization after combining all sources
...
# ── Model (unchanged from V12/V13/V15/V16/V17) ───────────────────────────────
...
def load_combined_data():
    """Load and combine WSPR + RBN Full + RBN DXpedition + Contest signatures.

    V18 CHANGE: Keep all median_snr in raw dB, apply global normalization at the end.
    """
    log.info(f"Connecting to ClickHouse at {CH_HOST}:{CH_PORT}...")
    client = clickhouse_connect.get_client(host=CH_HOST, port=CH_PORT)

    # ── WSPR Terrestrial Signatures (floor layer) ──
    ...
    # ── V18 CHANGE: Keep SNR in raw dB (no per-source normalization) ──
    log.info("Extracting raw SNR values (no per-source normalization)...")
    ...
    log.info(f"  WSPR raw dB:     mean={wspr_snr_raw.mean():.1f}, std={wspr_snr_raw.std():.1f}, range=[{wspr_snr_raw.min():.0f}, {wspr_snr_raw.max():.0f}]")
    log.info(f"  RBN Full raw dB: mean={rbn_full_snr_raw.mean():.1f}, std={rbn_full_snr_raw.std():.1f}, range=[{rbn_full_snr_raw.min():.0f}, {rbn_full_snr_raw.max():.0f}]")
    log.info(f"  RBN DX raw dB:   mean={rbn_dx_snr_raw.mean():.1f}, std={rbn_dx_snr_raw.std():.1f}, range=[{rbn_dx_snr_raw.min():.0f}, {rbn_dx_snr_raw.max():.0f}]")
    log.info(f"  Contest raw dB:  mean={contest_snr_raw.mean():.1f}, std={contest_snr_raw.std():.1f}, range=[{contest_snr_raw.min():.0f}, {contest_snr_raw.max():.0f}]")
    ...
    # ── Prepare weights (normalized within each source to mean=1) ──
    ...
    # ── V18 CHANGE: Storm upsample ──
    # kp_penalty = 1 - kp/9, so kp >= 5 means kp_penalty <= 4/9 = 0.444
    ...
    if storm_count > 0 and STORM_UPSAMPLE_FACTOR > 1:
        ...
        storm_y = y_combined_raw[storm_mask]
        storm_w = w_combined[storm_mask]

        log.info(f"  Upsampling storm rows {STORM_UPSAMPLE_FACTOR}x...")
        X_combined = np.vstack([X_combined] + [storm_X] * STORM_UPSAMPLE_FACTOR)
        y_combined_raw = np.concatenate([y_combined_raw] + [storm_y] * STORM_UPSAMPLE_FACTOR)
    ...
    # ── V18 CHANGE: Global normalization in raw dB ──
    log.info("Applying GLOBAL Z-score normalization (raw dB)...")
    global_mean = float(y_combined_raw.mean())
    global_std = float(y_combined_raw.std())

    log.info(f"  global_mean = {global_mean:.2f} dB")
    log.info(f"  global_std  = {global_std:.2f} dB")

    # Verify these are in expected ranges (from 9975WX spec)
    if not (-5 <= global_mean <= 5):
        log.warning(f"  WARNING: global_mean ({global_mean:.2f}) outside expected range [-5, +5]")
    if not (15 <= global_std <= 25):
        log.warning(f"  WARNING: global_std ({global_std:.2f}) outside expected range [15, 25]")

    # Z-normalize
    y_combined = ((y_combined_raw - global_mean) / global_std).reshape(-1, 1)
    w_combined = w_combined.reshape(-1, 1)

    log.info(f"  Normalized targets: mean={y_combined.mean():.4f}, std={y_combined.std():.4f}")

    # Show Z-values for each source's typical SNR (verification)
    wspr_typical_z = (-18.0 - global_mean) / global_std
    rbn_typical_z = (17.0 - global_mean) / global_std
    contest_ssb_z = (10.0 - global_mean) / global_std
    contest_rtty_z = (0.0 - global_mean) / global_std

    log.info(f"  Physical ordering check:")
    log.info(f"    WSPR -18 dB  → z = {wspr_typical_z:+.2f}σ (should be negative)")
    log.info(f"    RTTY   0 dB  → z = {contest_rtty_z:+.2f}σ")
    log.info(f"    SSB  +10 dB  → z = {contest_ssb_z:+.2f}σ")
    log.info(f"    RBN  +17 dB  → z = {rbn_typical_z:+.2f}σ (should be positive)")
    ...
    return X_combined, y_combined, w_combined, date_range, global_mean, global_std


# ── Training ─────────────────────────────────────────────────────────────────

def main():
    log.info(f"IONIS V18 Global Normalization Fix | {PHASE}")
    log.info(f"Device: {DEVICE} | Epochs: {EPOCHS} | Batch: {BATCH_SIZE}")
    log.info(f"Data sources: WSPR (floor) + RBN Full (middle) + RBN DX ({RBN_DX_UPSAMPLE_FACTOR}x) + Contest (ceiling)")
    log.info(f"Normalization: GLOBAL raw dB (V18 fix)")
    log.info(f"Storm upsample: {STORM_UPSAMPLE_FACTOR}x for Kp >= 5")
    log.info("")

    # ── Load Data ──
    X_np, y_np, w_np, date_range, global_mean, global_std = load_combined_data()

    log.info(f"Dataset: {len(X_np):,} rows x {INPUT_DIM} features")
    log.info(f"Global normalization: mean={global_mean:.2f} dB, std={global_std:.2f} dB")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'global_mean': global_mean,  # V18: raw dB mean
                'global_std': global_std,    # V18: raw dB std
                'dnn_dim': DNN_DIM,
                'sidecar_hidden': 8,
                'features': FEATURES,
                ...
                'data_sources': [
                    'wspr.signatures_v2_terrestrial (floor)',
                    'rbn.signatures (middle)',
                    'rbn.dxpedition_signatures (rare)',
                    'contest.signatures (ceiling)',
                ],
                'normalization': 'GLOBAL raw dB (V18 fix)',
                'contest_anchors': {'SSB': 10.0, 'RTTY': 0.0},
                'curriculum': 'Floor (-28 dB) + Middle (~18 dB) + Ceiling (+10 dB)',
    ...
    # Convert to dB using global_std
    log.info(f"  (≈ {(sfi_200 - sfi_70) * global_std:+.1f} dB SFI benefit)")
    log.info(f"  (≈ {(kp0 - kp9) * global_std:+.1f} dB storm cost)")
    ...
    log.info("")
    log.info("=== V18 NORMALIZATION FIX SUMMARY ===")
    log.info(f"V17 (broken):  global_mean=0.44σ (mixed Z), global_std=1.50σ")
    log.info(f"V18 (fixed):   global_mean={global_mean:.2f} dB, global_std={global_std:.2f} dB")
    log.info("")
    log.info("Denormalization formula:")
    log.info(f"  snr_dB = prediction_σ × {global_std:.2f} + {global_mean:.2f}")
    log.info("")
    log.info("Expected PSKR recall: 80-95% (not 99%)")
    log.info("Expected Step I recall: 85-95% (not 100%)")
    log.info("")
    log.info("Run validate_v18.py to test on contest paths.")
    log.info("Run validate_pskr.py to test on independent PSK Reporter data.")
